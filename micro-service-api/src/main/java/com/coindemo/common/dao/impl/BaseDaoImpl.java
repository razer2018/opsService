package com.coindemo.common.dao.impl;
...
import com.coindemo.common.constants.SqlId;
import com.coindemo.common.dao.BaseDao;
import com.coindemo.common.enums.DbExecuteType;
import com.coindemo.common.exception.DaoException;
import com.coindemo.common.id.IdGenerator;
import com.coindemo.common.model.Page;
import com.coindemo.common.model.PrimaryKeyObject;
import com.coindemo.common.query.Query;
import com.coindemo.common.utils.BeanUtils;
import com.coindemo.common.utils.GSONUtil;
...
public abstract class BaseDaoImpl<T extends PrimaryKeyObject<ID>, ID extends Serializable> implements BaseDao<T, ID> {
	...
	public T selectById(ID id) {
		...
		try {
			...
			throw new DaoException(String.format("根据ID查询对象出错！语句：%s"+e.getMessage(), getSqlName(SqlId.SQL_SELECT_BY_ID)), e);
		}
	}
	...
	public ID insert(T entity) {
		...
		try {
			...
			if(StringUtils.isEmpty(entity.getIsValid()))
				appendIsValid(entity);
			appendVersion(entity);
			sqlSessionTemplate.insert(getSqlName(SqlId.SQL_INSERT), entity);
			return entity.getId();
		} catch (Exception e) {
			throw new DaoException(String.format("添加对象出错！语句：%s"+e.getMessage(), getSqlName(SqlId.SQL_INSERT)), e);
		}
	}
	...
	public int updateByIdSelective(T entity) {
		...
		try {
			...
			throw new DaoException(String.format("根据ID更新对象某些属性出错！语句：%s", getSqlName("updateByPrimaryKeySelective")),
					e);
		}
	}
	...
	private final void appendIsValid(T entity){
		entity.setIsValid("1");
	}
	private final void appendVersion(T entity){
		entity.setVersion(StringUtils.isEmpty(entity.getVersion())?"1":Integer.toString((Integer.parseInt(entity.getVersion())+1)));
	}
